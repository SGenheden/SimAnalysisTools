"""
A class to help with the processing of simulation trajectories,
wrapper of a MDAnalysis package
"""
...
class TrajectoryProcessor(object):
    ...
    def append_action(self, action):
        ...
        if action.dosubsample : self.dosubsample
    ...
    def setup(self,printargs=False):
        ...
        description = "Analyze simulation trajectories"
        prog = sys.argv[0]
        if len(self.actions) == 1 :
            description = self.actions[0].descr()
            prog += " " + sys.argv[1]
            sys.argv.pop(1)

        self.argparser = argparse.ArgumentParser(description=description,
                            add_help=len(self.actions)==1, prog=prog)
